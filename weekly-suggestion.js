(() => {
  ...
  function purchaseBeans(isReorder = false) {
    const state = walletApi?.getState?.();
    const t = strings();
    if (!state) return;
    if (Number(state.balance) < BEANS_PRICE) {
      walletApi.toast(t.insufficientToast);
      return;
    }

    state.balance = Math.round((Number(state.balance) - BEANS_PRICE + Number.EPSILON) * 100) / 100;
    state.homeCoffeeUnlocked = true;
    state.transactions.unshift({
      id: globalThis.crypto?.randomUUID?.() || `beans-${Date.now()}`,
      type: 'purchase',
      description: t.transaction,
      amount: BEANS_PRICE,
      beanProductId: 'silvio-catuai-250',
      createdAt: new Date().toISOString()
    });
    state.transactions = state.transactions.slice(0, 50);
    walletApi.commit();
    walletApi.toast(isReorder ? t.reorderedToast : t.unlockedToast);
  }
  ...
  get('home-coffee-buy').addEventListener('click',()=>purchaseBeans(false));
  get('home-coffee-reorder').addEventListener('click',()=>purchaseBeans(true));
